...
for site, files in zip(ww_dict.keys(),ww_dict.values()):
    ...
    scaleddf['site'] = site
    scaleddf = scaleddf.set_index('site',append=True)
    
    df['site'] = site
    df = df.set_index('site',append=True)
    #------ build colormaps
    ...
    fig.tight_layout()
    plt.savefig(f'scaled_lineage_deconv{site}.pdf')
    plt.close()
    ...
    ax.legend(handles[::-1], labels[::-1],loc='center left', bbox_to_anchor=(0.3, -0.2),ncol=6)
    fig.tight_layout()
    plt.savefig(f'lineage_deconv{site}.pdf')
    plt.close()
